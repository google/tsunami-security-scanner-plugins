...
final class TestHelper {
  ...
  private static final ServerInterceptor mockServerInterceptor =
      mock(
  ...
  static DetectionReport buildValidDetectionReport(
      ExposedFlyteConsoleDetector detector,
      TargetInfo target,
  ...
  /**
   * Creates and returns a gRPC AdminServiceBlockingStub using an in-process server for testing
   * purposes.
   *
  ...
  private static AdminServiceBlockingStub getStubService() throws IOException {
    ...
    AdminServiceGrpc.AdminServiceBlockingStub blockingStub =
        AdminServiceGrpc.newBlockingStub(ClientInterceptors.intercept(channel));
    return blockingStub;
  }

  /**
   * Creates a mock instance of the FlyteProtoClient with a pre-configured gRPC stub service.
   *
   ...
   * @throws URISyntaxException If there is an error in the URI syntax during the setup.
   */
