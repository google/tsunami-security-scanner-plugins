...
public final class PapercutNGMFVulnDetectorTest {
  ...
  public void setUp() {
    ...
    papercutService = NetworkService
            .newBuilder()
            .setNetworkEndpoint(
  ...
  public void detect_whenNotVulnerable_returnsNoFinding() throws IOException {
    ...
    mockWebServer.enqueue(
      new MockResponse().setResponseCode(302)
    );
    mockWebServer.url("/app");
    ...
    mockWebServer.enqueue(
      new MockResponse()
          .setResponseCode(200)
          .setBody(loadResource("nonvulnerable_page.html"))
    );
    mockWebServer.url("/app");
    ...
    assertThat(
        detector.detect(
          TargetInfo
            .newBuilder()
            .addNetworkEndpoints(forHostnameAndPort(
