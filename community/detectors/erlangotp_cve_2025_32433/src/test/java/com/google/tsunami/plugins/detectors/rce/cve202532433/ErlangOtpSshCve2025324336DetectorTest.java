...
/** Unit tests for {@link ErlangOtpSshCve2025324336Detector}. */
@RunWith(JUnit4.class)
public class ErlangOtpSshCve2025324336DetectorTest {
  ...
  @Inject private ErlangOtpSshCve2025324336Detector detector;
  ...
  public void detect_whenVulnerable_returnsVulnerability() throws Exception {
    ...
    NetworkService service =
        NetworkService.newBuilder()
            ...
            .setSoftware(Software.newBuilder().setName("ssh"))
            .setServiceName("ssh")
            .build();
  ...
  @Test
  public void detect_whenTargetNotErlangOtpSsh_returnsNoVulnerability() throws Exception {
    // Make target info
    NetworkService service =
        NetworkService.newBuilder()
            .setNetworkEndpoint(NetworkEndpointUtils.forIpAndPort("127.0.0.1", 10911))
  ...
  private DetectionReport getVulnerableDetectionReport(
      ErlangOtpSshCve2025324336Detector detector, NetworkService service, TargetInfo targetInfo) {
  ...
  private void configureMockSocket(boolean isErlanOtp) throws IOException {
      ...
      Mockito.when(socket.getInputStream())
          .thenAnswer(
              (Answer<ByteArrayInputStream>)
                  invocation ->
                      new ByteArrayInputStream("SSH-2.0-Erlang/".getBytes(StandardCharsets.UTF_8)));
    } else {
      Mockito.when(socket.getInputStream())
          .thenAnswer(
              (Answer<ByteArrayInputStream>)
