...
/** Create a signature V4 authentication bypass for an HTTP request. */
public class Signer {
  //
  ...
  private void setCanonicalHeaders(Set<String> ignoredHeaders) {
  ...
  private void setCanonicalRequest() throws NoSuchAlgorithmException {
    setCanonicalHeaders(skipHeaders);
  ...
  public static HttpRequest signV4(
    ...
    ZonedDateTime date =
        ZonedDateTime.parse(request.headers().get("x-amz-date").get(), Time.AMZ_DATE_FORMAT);
