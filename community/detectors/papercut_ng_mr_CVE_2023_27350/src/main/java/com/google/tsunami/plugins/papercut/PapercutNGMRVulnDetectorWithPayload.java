...
@PluginInfo(
    ...
    bootstrapModule = PapercutNGMRVulnDetectorWithPayloadBootstrapModule.class)
...
public final class PapercutNGMRVulnDetectorWithPayload implements VulnDetector {
  ...
  private String JSESSIONID_COOKIE; // <-- this will be used in RCE (Java Jar and Command) checks

  // Used as FormData in 'isServiceVulnerable'
  private final static String PRECHECK_PAYLOAD = "{" +
  ...
  @Inject
  PapercutNGMRVulnDetectorWithPayload(
  ...
  // Use a 'GET' request to check to see if it is a vulnerable version
  private boolean isServiceVulnerablePreCheck(NetworkService networkService) {
    String targetUri = NetworkServiceUtils.buildWebApplicationRootUrl(networkService) +
            "/app?service=page/SetupCompleted";
     try {
       // Checking if application is vulnerable with 'GET' request to "/app?service=page/SetupCompleted"
        HttpResponse response = httpClient.send(get(targetUri).build(), networkService);

        // If the response succeeded, check for the version Number
        if (response.status().code() == HttpStatus.OK.code()) {

          if (response.bodyString().isEmpty()) {
            logger.atWarning().log("No content was found for %s", response.responseUrl().toString());
            return false;
          } // End isEmpty 'if'

          // Use Regex to retrieve the PaperCut NR/MF Version Number
          String content = response.bodyString().get();
          Matcher reMatcher = Pattern.compile("<span>\\d+\\.\\d+\\.\\d+</span>", Pattern.CASE_INSENSITIVE).matcher(content);
          if (reMatcher.find()) {
            //
            String VULNERABLE_INSTANCE_VERSION = reMatcher.group(1);
            logger.atWarning().log("Found potentially vulnerable version of PaperCut NG/MF: %s", VULNERABLE_INSTANCE_VERSION);
            return true;
          } // End reMatcher 'if'
        ...
        logger.atWarning().withCause(err).log("Unable to send request '%s'", targetUri);
        return false;
    } // End 'catch'

      // If none of the above statements are true, default to 'false'
      return false;
  }
  ...
  private boolean isServiceVulnerable(NetworkService networkService) {
    ...
    PayloadGeneratorConfig config =
        PayloadGeneratorConfig.newBuilder()
            ...
            .setInterpretationEnvironment(PayloadGeneratorConfig.InterpretationEnvironment.LINUX_SHELL)
