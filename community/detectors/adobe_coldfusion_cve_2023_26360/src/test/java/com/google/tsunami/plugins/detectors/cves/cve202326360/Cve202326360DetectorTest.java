...
public final class Cve202326360DetectorTest {
  ...
  public void setUp() throws IOException {
    ...
    service =
        NetworkService.newBuilder()
            .setNetworkEndpoint(forHostnameAndPort("localhost", 8500))
    ...
    targetInfo =
        TargetInfo.newBuilder().addNetworkEndpoints(forHostnameAndPort("localhost", 8500)).build();
  }
  ...
  public void detect_whenVulnerable_returnsDetection() {
    ...
    //   Request notVulnerableRequest =
    //   new Request.Builder()
    //       .url("http://google.com/")
    //       .build();
    //   try (Response response = httpClient.newCall(notVulnerableRequest).execute()) {
    //   }catch (IOException e) {
    //       e.printStackTrace();
    //   }
