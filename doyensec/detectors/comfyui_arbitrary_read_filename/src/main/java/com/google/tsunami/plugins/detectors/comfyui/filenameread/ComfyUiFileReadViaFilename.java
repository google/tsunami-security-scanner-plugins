...
@PluginInfo(
    ...
    name = "ComfyUI_fileread_filename",
    version = "0.1",
    description =
        "This plugin detects an unauthenticated file read in ComfyUI Manager by abusing a path"
            + " traversal on the filename field of the install_module endpoint.",
    author = "Savino Sisco (savio@doyensec.com), Leonardo Giovannini (leonardo@doyensec.com)",
    bootstrapModule = ComfyUiFileReadViaFilenameBootStrapModule.class)
public final class ComfyUiFileReadViaFilename implements VulnDetector {
  ...
  static final String VULNERABILITY_REPORT_TITLE =
      "Arbitrary File Read in ComfyUI Manager via filename Field";
  ...
  @VisibleForTesting
  static final String FILE_TO_LEAK_WIN = "file:C:\\Windows\\system32\\drivers\\etc\\hosts";
  ...
  @Inject
  ComfyUiFileReadViaFilename(@UtcClock Clock utcClock, HttpClient httpClient) {
  ...
  private boolean isComfyUi(NetworkService networkService) {
    ...
    try {
      ...
      targetUri = NetworkServiceUtils.buildWebApplicationRootUrl(networkService) + VERSION_ENDPOINT;
      req =
          HttpRequest.get(targetUri)
              .setHeaders(HttpHeaders.builder().addHeader("Accept", "application/json").build())
              .build();
  ...
  private Optional<String> findFrontendWebroot(NetworkService networkService) {
    ...
    try {
      ...
      try {
        JsonArray logs =
            response.bodyJson().get().getAsJsonObject().get("entries").getAsJsonArray();

        for (JsonElement log : logs) {
          String message = log.getAsJsonObject().get("m").getAsString();
  ...
  private boolean isServiceVulnerable(NetworkService networkService) {
    ...
    String outputFilename =
        ComfyUiFileReadViaFilename.generateRandomString(OUTPUT_FILENAME_LENGTH) + ".safetensors";

    String fileUri;
    String outputPath;
    if (os.equals("posix")) {
      fileUri = FILE_TO_LEAK_POSIX;
      outputPath = webroot + "/" + WEBROOT_SUBDIRECTORY + "/" + outputFilename;
    ...
    String payload =
        PAYLOAD_TEMPLATE.replace("OUTPUT_FILE", outputPath).replace("FILE_TO_LEAK", fileUri);
    ...
    try {
      ...
      targetUri =
          NetworkServiceUtils.buildWebApplicationRootUrl(networkService) + QUEUE_START_ENDPOINT;
