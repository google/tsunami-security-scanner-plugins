...
TMP_DATA="/tmp/dolibarr_fingerprints"
# Path to the local git repository for dolibarr codebase.
...
BINPROTO="${PROJECT_ROOT}/src/main/resources/fingerprinters/web/data/community/dolibarr.binproto"

# Convert the existing data file to a human-readable json file.
convertFingerprint "${BINPROTO}"  "${JSON_DATA}"


# Fetch dolibarr codebase.
if [[ ! -d "${GIT_REPO}" ]] ; then
  git clone https://github.com/Dolibarr/dolibarr.git "${GIT_REPO}"
fi
...
echo "Fingerprint updated for dolibarr. Please commit the following file:"
echo "  ${BIN_DATA}"
echo "to"
echo "  ${BINPROTO}"
