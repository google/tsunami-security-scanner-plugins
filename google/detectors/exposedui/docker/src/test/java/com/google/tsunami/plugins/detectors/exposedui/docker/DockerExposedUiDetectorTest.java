...
/** Tests for {@link DockerExposedUiDetector}. */
@RunWith(JUnit4.class)
public final class DockerExposedUiDetectorTest {
  ...
  @Inject private DockerExposedUiDetector detector;
  ...
  public void setUp() {
    ...
    Guice.createInjector(
            ...
            new DockerExposedUiDetectorBootstrapModule())
        .injectMembers(this);
  }
  ...
  @Test
  public void detect_whenResponseIsMisconfiguredDocker_reportsVuln() throws IOException {
    String unauthenticatedRootApi =
        Resources.toString(Resources.getResource(this.getClass(), "testdata/root.json"), UTF_8);
    String validApiResponse =
        Resources.toString(Resources.getResource(this.getClass(), "testdata/version.json"), UTF_8);
    mockWebServer.setDispatcher(
        new FakeDockerDispatcher("", unauthenticatedRootApi, validApiResponse));
    mockWebServer.start();
    ...
    ImmutableList<NetworkService> httpServices =
        ImmutableList.of(
            NetworkService.newBuilder()
                ...
                .setSoftware(Software.newBuilder().setName("Docker"))
                .setServiceName("http")
                .build());
  ...
  @Test
  public void detect_whenNonDockerWebApp_ignoresServices() throws IOException {
  ...
  @Test
  public void detect_whenDockerWebAppReturnInvalidResponse_ignoresServices() throws IOException {
        ...
        ImmutableList.of(
            NetworkService.newBuilder()
                .setNetworkEndpoint(
                    forHostnameAndPort(mockWebServer.getHostName(), mockWebServer.getPort()))
                .setTransportProtocol(TransportProtocol.TCP)
                .setSoftware(Software.newBuilder().setName("Docker remote API"))
                .setServiceName("docker")
                .build());
  ...
  static final class FakeDockerDispatcher extends Dispatcher {
    private final String applicationRoot;
    private final String apiPageResponse;
    private final String podsPageResponse;

    FakeDockerDispatcher(String applicationRoot, String apiPageResponse, String podsPageResponse) {
      this.applicationRoot = checkNotNull(applicationRoot);
      this.apiPageResponse = checkNotNull(apiPageResponse);
      this.podsPageResponse = checkNotNull(podsPageResponse);
    }

    @Override
    public MockResponse dispatch(RecordedRequest recordedRequest) {
      if (recordedRequest.getPath().equals(applicationRoot + "/")) {
        return new MockResponse()
            .setResponseCode(HttpStatus.NOT_FOUND.code())
            .setHeader(HttpHeaders.CONTENT_TYPE, MediaType.JSON_UTF_8)
            .setBody(apiPageResponse);
      }

      if (recordedRequest.getPath().equals(applicationRoot + "/version")) {
        return new MockResponse()
            .setResponseCode(HttpStatus.OK.code())
            .setHeader(HttpHeaders.CONTENT_TYPE, MediaType.JSON_UTF_8)
            .setBody(podsPageResponse);
      }
