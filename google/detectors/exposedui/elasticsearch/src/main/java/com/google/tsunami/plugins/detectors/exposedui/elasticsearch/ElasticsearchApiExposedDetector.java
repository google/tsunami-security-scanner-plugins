...
/** A {@link VulnDetector} that detects exposed Elasticsearch REST API endpoints. */
@PluginInfo(
    type = PluginType.VULN_DETECTION,
    name = "ElasticsearchApiExposedDetector",
    version = "0.1",
    description = "This plugin detects exposed Elasticsearch REST API endpoints.",
    author = "Tsunami Team (tsunami-dev@google.com)",
    bootstrapModule = ElasticsearchApiExposedDetectorBootstrapModule.class)
public final class ElasticsearchApiExposedDetector implements VulnDetector {
  ...
  @Inject
  ElasticsearchApiExposedDetector(@UtcClock Clock utcClock, HttpClient httpClient) {
  ...
  /** Checks if a {@link NetworkService} has a Elasticsearch REST API endpoint exposed. */
  ...
  private DetectionReport buildDetectionReport(
    ...
    TextData details =
        TextData.newBuilder()
            .setText(
                String.format(
                    "The Elasticsearch REST API endpoint at %s is exposed.",
