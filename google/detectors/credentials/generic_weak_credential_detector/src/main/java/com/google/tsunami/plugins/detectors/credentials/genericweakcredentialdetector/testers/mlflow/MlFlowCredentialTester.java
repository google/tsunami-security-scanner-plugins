    ...
    private boolean isMlFlowAccessible(NetworkService networkService, TestCredential credential) {
        ...
        try {
            ...
            return response.status().isSuccess()
                    && response
                    .bodyString()
                    .map(MlFlowCredentialTester::bodyContainsSuccessfulUserRegistration)
                    .orElse(false);
        } catch (IOException e) {
                ...
                post(url)
                        ...
                        .setRequestBody(
                                ByteString.copyFromUtf8(
                                        "{\"username\": \"googleTsunamiSecurityScanner\", \"password\": \"googleTsunamiSecurityScanner\"}"))
                        .build());
    }

    /**
     * A successful authenticated request to the /api/2.0/mlflow/users/create endpoint returns a JSON
     * with a root key like the following:
    ...
    private static boolean bodyContainsSuccessfulUserRegistration(String responseBody) {
        try {
            ...
            if (response.has("user")) {
                logger.atInfo().log("Successfully created a new mlflow user as an admin");
                return true;
            } else {
                return false;
            }
